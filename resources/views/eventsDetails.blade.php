                        ...
                        <div class="mb-8">
                            <div class="flex items-center">
                                ...
                                {{-- <div class="ml-auto flex items-center">
                                    <a href="{{route('eventsDetails', $events->id)}}" title="view details" class="mr-4">
                            ...
                            <p class="text-gray-900 text-base font-bold">{{$event->location}}</p>
                        </div>
                        <div class="flex items-center">
                            <img class="w-10 h-10 rounded-full mr-4" src="{{asset(''. $event->users->photo)}}" alt="Avatar of Jonathan Reinink">
                            <div class="text-sm"> {{-- xxxxxxxxxxxxxxxxxxxxxxxxxx look here! xxxxxxxxxxxxxxxxxxxxxxxxxxx--}}
                                <form action="{{route('organiser.dashboard')}}">
                                    <button class="text-green-600 font-bold leading-none">{{$event->users->name}}</button>
                                </form>
                                <p class="text-gray-600">{{$event->available_seats}}</p>
                                ...
                                {{-- <p class="text-red-600 text-right font-bold">{{$event->acceptance}}</p> --}}
                                <p class="text-red-600 text-right font-bold">{{$event->categories->name}}</p>
                            </div>
                            <form action="{{route('apply', $event->id)}}" method="GET">
                                @csrf
                                <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow mt-5 ml-20">
                                    Apply for the event
                                </button>
                            </form>
                        </div>
